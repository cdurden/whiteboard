                    ...
                    $scope.$on("resetBackgrounds", function (ev, msg) {
                        if (Array.isArray(msg.target)) {
                            msg.target.forEach(function (target) {
                                $scope.$broadcast("resetTargetBackground", {
                                    target: target,
                                });
                            });
                        } else {
        ...
        return {
            ...
            link: function (scope, element, attrs, ctrl) {
                element.bind("touchstart", function (ev) {
                    ...
                    scope.$emit("toggleAllSubmenu", {
                        action: "hide",
                        level: "2",
                    });
                    scope.$broadcast("toggleSubmenu", {
                        action: "show",
                        level: "2",
                    });
                });
                element.bind("mouseover", function (ev) {
                    if (
                        ev.buttons === 0 &&
                        (angular.element(ev.relatedTarget).is("svg") ||
                            ev.relatedTarget?.raphael)
                    ) {
                        ctrl.menuHandler("show");
                    }
                });
        ...
        return {
            ...
            link: function (scope, element, attrs, submenuOpenerCtrl) {
                var bindMouseEv = function () {
                    element.bind("mouseover", function (ev) {
                        submenuOpenerCtrl.submenuOpener({
                            action: "show",
                            level: attrs.wbLevel,
                        });
                    });
                    element.bind("mouseleave", function (ev) {
                        ...
                        if (
                            angular.element(ev.relatedTarget).hasClass("lvl1") // FIXME: This is a p element that does not fill the box for the menu item
                        ) {
                            console.log("A");
                            submenuOpenerCtrl.submenuCloser({
                                action: "hide",
                                level: "2",
                            });
                        } else if (
                            angular.element(ev.relatedTarget).hasClass("lvl2")
                        ) {
                            console.log("B");
                            ...
                            angular
                                .element(ev.relatedTarget)
                                .hasClass("wb-submenu-opener")
                        ) {
                            console.log("C");
    ...
    .directive("wbSubmenuItems", function () {
        return {
            ...
            controller: function ($scope, BoardData) {
                ...
                this.changeSetting = function (attrs) {
                    if (attrs.wbColor) {
                        this.setColors(attrs.wbColorType, attrs.wbColor);
                        $scope.$emit("activateMenu", "hide");
                    } else if (attrs.wbThickness) {
                        this.setThickness(attrs.wbThickness);
                        $scope.$emit("activateMenu", "hide");
                    } else if (attrs.wbTool) {
                        $scope.$emit("setCursorClass", { tool: attrs.wbTool });
                        this.setTool(attrs.wbTool);
                        $scope.$emit("activateMenu", "hide");
                    }
                };
            },
            link: function (scope, element, attrs, submenuItemsCtrl) {
                ...
                element.bind("touchstart", function (ev) {
                    ev.stopPropagation();
                    if (
                        angular.element(ev.target).hasClass("menu") ||
                        angular.element(ev.target).hasClass("icon")
                    ) {
                        scope.$emit("toggleAllSubmenu", {
                            action: "hide",
                            level: (Number(attrs.wbLevel) + 1).toString(),
                        });
                        scope.$broadcast("toggleSubmenu", {
                            action: "show",
                            level: (Number(attrs.wbLevel) + 1).toString(),
                        ...
                        submenuItemsCtrl.changeSetting(attrs);
                    }
                });

                element.bind("mouseleave", function (ev) {
                    ev.stopPropagation();
                    // The mouseleave event's relatedTarget property is
                    // the EventTarget that the pointing device entered to
                    // See https://developer.mozilla.org/en-US/docs/Web/API/MouseEvent/relatedTarget
                    if (angular.element(ev.relatedTarget).is("svg")) {
                        console.log("F");
                        submenuItemsCtrl.changeSetting(attrs);
                    } else if (ev.relatedTarget?.raphael) {
                        console.log("G");
                        submenuItemsCtrl.changeSetting(attrs);
                    } else if (
                        angular.element(ev.relatedTarget).hasClass("menu") ||
                        angular.element(ev.relatedTarget).hasClass("icon")
                    ) {
                        // console.log(ev)
                        scope.$emit("toggleAllSubmenu", {
                            action: "hide",
        ...
        return {
            ...
            controller: function ($scope) {
                ...
                this.calcBg = function (mouseX, leftOffset) {
                    ...
                    var bgSizes = {};
                    bgSizes.overed =
                        ((mouseX - leftOffset) * 100) / this.getElemWidth();
            ...
            link: function (scope, element, attrs, ctrl) {
                ...
                var setBg = function (el, sizes) {
                    ...
                    el.css({
                        background:
                            "linear-gradient(90deg, rgba(177,102,24,0.96) " +
                            sizes.overed +
                            "%, rgba(53,53,53,0.93) 0%)",
                    });
                };
                ...
                element.bind("mouseover", function (ev) {
                    ...
                    if (
                        angular
                            ...
                            .hasClass("level-two-items")
                    ) {
                        var $levelOne = angular
                            .element(ev.currentTarget)
                            .parents(".level-one");
                        // console.log($levelOne)
                        scope.$emit("resetBackgrounds", {
                            target: "level-two-items",
                        });
                        setBg($levelOne, { overed: 100 });
                    } else if (
                        angular.element(ev.currentTarget).hasClass("level-one")
                    ) {
                        scope.$emit("resetBackgrounds", {
                            target: "level-two-items",
                        });
                        ...
                        angular
                            .element(ev.currentTarget)
                            .hasClass("color-palette")
                    ) {
                        // console.log('A')
                        var $levelTwo = angular
                            .element(ev.currentTarget)
                            .parents(".level-two-items");
                        setBg($levelTwo, { overed: 100 });
                        scope.$emit("resetBackgrounds", {
                            target: "level-three-items",
                        });
                    } else if (
                        angular.element(ev.currentTarget).hasClass("thickness")
                    ) {
                        // console.log('A')
                        var $levelTwo = angular
                            .element(ev.currentTarget)
                ...
                element.bind("mousemove", function (ev) {
                    ...
                    // console.log('ev');
                    if (
                        $el.hasClass("level-one") ||
                        $el.hasClass("level-two-items") ||
                        $el.hasClass("thickness")
                    ) {
                        // console.log('over level one');
                        var bgSizes = ctrl.calcBg(
                            ev.clientX,
                            $el.offset().left
                        );
                        setBg($el, bgSizes);
                ...
                element.bind("mouseleave", function (ev) {
                    ...
                    var $elToElement = angular.element(ev.relatedTarget);
                    ...
                    if ($elTarget.hasClass("level-two-items")) {
                        ...
                        $elTarget.hasClass("color-palette") ||
                        $elTarget.hasClass("thickness")
                    ) {
                        if (
                            $elToElement.is("svg") ||
                            ev.relatedTarget?.raphael
                            ...
                            $elToElement.hasClass("wb-submenu-opener") ||
                            $elToElement.hasClass("level-three-items")
                        ) {
                            // scope.$emit('resetBackgrounds', {target: 'all'});
                            scope.$emit("resetBackgrounds", {
                                target: ["color-palette", "thickness"],
                            });
                        }
                    }
                });
                ...
                scope.$on("resetTargetBackground", function (ev, msg) {
                    ...
                    if (msg.target === "all") {
                        // console.log('- ', element)
                        element.hasClass("color-palette")
                            ? setColorBg(element, scope.color, { overed: 0 })
                            : setBg(element, { overed: 0 });
                    } else if (
                        element.hasClass("color-palette") &&
                        (msg.target === "level-three-items" ||
                            msg.target === "color-palette")
                    ) {
                        // console.log(scope.color)
                        setColorBg(element, scope.color, { overed: 0 });
                    } else if (
                        element.hasClass("thickness") &&
                        (msg.target === "level-three-items" ||
                            msg.target === "thickness")
                    ) {
                        setBg(element, { overed: 0 });
