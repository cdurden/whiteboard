angular.module("whiteboard.services.shapebuilder", []).factory("ShapeBuilder", [
    "BoardData",
    "Snap",
    function (BoardData, Snap) {
        function setColor(shape, colors) {
            if (shape.type === "path") {
                shape.attr("stroke", colors.stroke);
            } else if (
                shape.type === "text" &&
                shape.attr("text") === "Start Typing..."
            ) {
        ...
        function drawExistingPath(shape) {
            ...
            var existingPath = BoardData.getShapeById(
                shape.myid,
                shape.socketId
            );
        ...
        function newShape(id, socketId, tool, x, y) {
            var shapeConstructors = {
                ...
                text: function (x, y, text) {
                    return BoardData.getBoard().text(x, y, text).attr({
                        "font-size": 18,
            ...
            var shape = !!tool.text
                ? shapeConstructors["text"](x, y, tool.text)
                : shapeConstructors[tool.name](x, y);
            shape.initX = x;
            shape.initY = y;
            shape.tool = tool;
